...
forecast_out = int(math.ceil(0.1 * len(df))) # number of days out
print(forecast_out)

df['label'] = df[forecast_col].shift(-forecast_out)

X = np.array(df.drop(['label', 'Adj. Close'], 1)) # features, features will be everything BUT the label column
